...
image_folder = os.path.join(os.path.dirname(os.path.abspath(__file__)), "wildfire_images")
os.makedirs(image_folder, exist_ok=True)

# Function to capture multiple images over a 1-minute period
def capture_images_for_duration(alert_folder, payload, duration_seconds=60, interval_seconds=5):
...
def on_message(client, userdata, msg):
    try:
        ...
        if msg.topic == "esp32_01/wildfire/alert" and payload.get("wildfire_detected", False):
...
def on_connect(client, userdata, flags, rc):
    ...
    client.subscribe("esp32_01/wildfire/alert")
    print("Subscribed to esp32_01/wildfire/alert")
